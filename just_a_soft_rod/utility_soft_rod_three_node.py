...
def fun(qOld, dt, q0, m, u, nv, visc, R1, R2, R3, mg, 
        ScaleSolver, 
        tol, 
        maximum_iter, 
        EI, 
    ...
    while (normf > tol * ScaleSolver):
        ...
        #   forces
        Fb, Jb = getFb(qNew, EI, deltaL) # ISSUE: Jb is not correct
        Fs, Js = getFs(qNew, EA, deltaL, nv)
        Fv, Jv = getFv(qNew, nv, visc, q0, dt, R1, R2, R3)
        Fg = getFg(mg)
...
def getFb(q, EI, deltaL):
    ...
    xkm1, ykm1, xk, yk, xkp1, ykp1 = q[0,0], q[1,0], q[2,0], q[3,0], q[4,0], q[5,0]
    gradEnergy = gradEb(xkm1, ykm1, xk, yk, xkp1, ykp1, 0, deltaL, EI) # gradEb returns the gradient of (curvature)^2

    Fb = Fb - gradEnergy
    
    # Compute Jacobian of bending force
    Jb = np.zeros((len(q), len(q)))
    hessEnergy = hessEb(xkm1, ykm1, xk, yk, xkp1, ykp1, 0, deltaL, EI) # hessEb returns the hessian of (curvature)^2
    Jb = Jb - hessEnergy
...
def getFv(q, nv, visc, q0, dt, R1, R2, R3):
    ...
    Jv[0:2, 0:2] = R1 * Jv[0:2, 0:2]
    Jv[2:4, 2:4] = R2 * Jv[2:4, 2:4]
    Jv[4:6, 4:6] = R3 * Jv[4:6, 4:6]
